...
@implementation YZHGraphicsBeginInfo
...
@implementation YZHGraphicsContext

-(instancetype)init
{
    self = [super init];
    if (self) {
        self.imageAlignment = YZHGraphicsImageAlignmentLeft;
        self.ctx = NULL;
    }
    return self;
}
...
-(UIImage*)createGraphicesImageWithStrokeColor:(UIColor*)strokeColor
{
    ...
    if (self.graphicsBeginBlock) {
        ...
        YZHGraphicsBeginInfo *beginInfo = self.beginInfo;
        if (beginInfo) {
            ...
            beginInfo = [[YZHGraphicsBeginInfo alloc] initWithGraphicsSize:size opaque:opaque scale:scale lineWidth:lineWidth];
...
+(UIImage*)createCrossImageWithSize:(CGSize)size lineWidth:(CGFloat)lineWidth backgroundColor:(UIColor*)backgroundColor strokeColor:(UIColor*)strokeColor
{
    ...
    return [YZHGraphicsContext createCrossImageWithSize:size lineWidth:lineWidth backgroundColor:backgroundColor strokeColor:strokeColor transform:transform];
}
...
+(UIImage*)_createArrowImageWithType:(YZHArrowDirection)type size:(CGSize)size basePointShitPoint:(CGPoint)basePointShift topPointShift:(CGFloat)topPointShift lineWidth:(CGFloat)lineWidth backgroundColor:(UIColor*)backgroundColor strokeColor:(UIColor*)strokeColor
{
    if (CGSizeEqualToSize(size, CGSizeZero)) {
        return nil;
    }
    ...
    YZHGraphicsContext *ctx = [[YZHGraphicsContext alloc] initWithBeginBlock:^(YZHGraphicsContext *context) {
        ...
        else if (type == YZHArrowDirectionLeft) {
        ...
        else if (type == YZHArrowDirectionDown) {
        ...
        else if (type == YZHArrowDirectionRight) {
...
{
    if (type < YZHArrowDirectionUp || type > YZHArrowDirectionRight || angle >= M_PI || angle == 0) {
        return nil;
    }
    ...
    CGSize size = CGSizeZero;
    if (type == YZHArrowDirectionUp || type == YZHArrowDirectionDown) {
        size = CGSizeMake(w, h);
    }
    else if (type == YZHArrowDirectionLeft || type == YZHArrowDirectionRight) {
        size = CGSizeMake(h, w);
    }
    
    return [YZHGraphicsContext _createArrowImageWithType:type size:size basePointShitPoint:CGPointMake(basePointHShift, basePointVShift) topPointShift:topPointVShift lineWidth:lineWidth backgroundColor:backgroundColor strokeColor:strokeColor];
}
...
+(UIImage*)createArrowImageWithType:(YZHArrowDirection)type arrowAngle:(CGFloat)angle baseHeight:(CGFloat)baseHeight lineWidth:(CGFloat)lineWidth backgroundColor:(UIColor*)backgroundColor strokeColor:(UIColor*)strokeColor
{
    ...
    return [YZHGraphicsContext createArrowImageWithType:type arrowAngle:angle baseWidth:baseWidth lineWidth:lineWidth backgroundColor:backgroundColor strokeColor:strokeColor];
    return nil;
}
...
{
    if (CGSizeEqualToSize(size, CGSizeZero) || type < YZHArrowDirectionUp || type > YZHArrowDirectionRight) {
        return nil;
    }
    
    CGFloat baseWidth = size.width;
    CGFloat baseHeight = size.height;
    if (type == YZHArrowDirectionLeft || type == YZHArrowDirectionRight) {
        baseWidth = size.height;
        baseHeight = size.width;
    }
    CGFloat atanValue = baseWidth/(2 * baseHeight);
    CGFloat arrowAngle = 2 * atan(atanValue);
    return [YZHGraphicsContext createArrowImageWithType:type arrowAngle:arrowAngle baseWidth:size.width lineWidth:lineWidth backgroundColor:backgroundColor strokeColor:strokeColor];
}
...
+(UIImage*)graphicesImage:(UIImage*)image cornerRadius:(CGFloat)cornerRadius borderWidth:(CGFloat)borderWidth borderColor:(UIColor*)borderColor
{
    ...
    return [YZHGraphicsContext graphicesImage:image graphicsSize:size inRect:CGRectMake(0, 0, size.width, size.height) cornerRadius:cornerRadius borderWidth:borderWidth borderColor:borderColor];
}
...
+(UIImage*)graphicesImage:(UIImage*)image inRect:(CGRect)rect cornerRadius:(CGFloat)cornerRadius borderWidth:(CGFloat)borderWidth borderColor:(UIColor*)borderColor
{
    ...
    return [YZHGraphicsContext graphicesImage:image graphicsSize:size inRect:rect cornerRadius:cornerRadius borderWidth:borderWidth borderColor:borderColor];
}
...
{
    return [YZHGraphicsContext graphicesImage:image graphicsSize:graphicsSize inRect:rect byRoundingCorners:UIRectCornerAllCorners cornerRadius:cornerRadius borderWidth:borderWidth borderColor:borderColor];
}
