...
typedef NS_ENUM(NSInteger, YZHProgressViewStyle)
{
//    YZHProgressViewStyleNULL          = -1,
    YZHProgressViewStyleIndicator     = 0,
};

@class YZHProgressView;
typedef void(^YZHProgressTimeoutBlock)(YZHProgressView *progressView);
typedef void(^YZHProgressDismissCompletionBlock)(YZHProgressView *progressView,NSInteger dismissTag, BOOL finished);

@interface YZHProgressView : UIView
...
/** 最大适配的大小，默认为（SCREEN_WIDTH * 0.8，SCREEN_HEIGHT * 0.9） */
@property (nonatomic, assign) CGSize maxFitSize;
...
@property (nonatomic, assign) YZHProgressViewStyle progressViewStyle;
...
@property (nonatomic, copy) YZHProgressTimeoutBlock timeoutBlock;
//此completionBlock为在dismiss后回调
@property (nonatomic, copy) YZHProgressDismissCompletionBlock completionBlock;
