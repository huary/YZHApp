...
@interface YZHCalendarItemCell ()

/* <#注释#> */
@property (nonatomic, strong) YZHButton *item;

@end

@implementation YZHCalendarItemCell
...
-(void)_setupChildView
{
    ...
    UIImage *bgImage = [YZHGraphicsContext createImageWithSize:CGSizeMake(w, h) cornerRadius:h/2 borderWidth:0 borderColor:nil backgroundColor:RGB_WITH_INT_WITH_NO_ALPHA(0x388CFF)];
    
    self.item = [YZHButton buttonWithType:UIButtonTypeCustom];
...
-(void)_updateDot
{
    if (self.itemModel.haveBottomDot) {
        CGFloat dotH = dotH_s;
        UIImage *image = [YZHGraphicsContext createImageWithSize:CGSizeMake(dotH, dotH) cornerRadius:dotH/2 borderWidth:0 borderColor:nil backgroundColor:RGB_WITH_INT_WITH_NO_ALPHA(0x388CFF)];
